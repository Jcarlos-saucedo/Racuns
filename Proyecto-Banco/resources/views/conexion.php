	...
	function InsertarDatos($cantidad, $time, $period,$user, $apelli)
	{
		...
		$iddd=0;
	  	...
	  	while ($persona= $query_usuario->fetch()){
	  		...
	  		{
	  			$iddd=$persona[0];
	  			$conex_calculo="UPDATE usuarios SET cantidadd='".$cantidad."', years='".$time."', fecha='".$period."' WHERE ID='".$iddd."'";
				$query_update=$conectar->prepare($conex_calculo);
				...
				while($fila=$query_productos->fetch()){
				...
				$pdf->AddPage();
				$pdf->Image("imagenes/PHP/hstw.png", 80, 10, 60);
				$pdf->Ln(25);
				...
				$pdf->Cell(50,10,$apelli, 1, 0,'C');
				$pdf->Cell(40,10,"", 0, 0);
				$pdf->Cell(50,10,"".$time." Years", 1, 1, 'C');
